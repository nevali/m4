...
if ! test x"$XCRUN" = x"" ; then
	...
	if ! test x"$XCSDK" = x"" ; then
		...
		test -z "$CXX" && CXX="$XCRUN -sdk $XCSDK clang++"
		test -z "$CXXCPP" && CXXCPP="$XCRUN -sdk $XCSDK clang++ -E"
		test -z "$OBJC" && OBJC="$XCRUN -sdk $XCSDK clang"
		test -z "$OBJCXX" && OBJCXX="$XCRUN -sdk $XCSDK clang++"
